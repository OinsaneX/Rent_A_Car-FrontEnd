...
export default function index() {
  ...
  const onSubmit = async () => {
    if (!rentData.location) {
      ...
      NotificationManager.error("Introduzca la hora de Entrega", "Error", 2000);
    } else if (
      (new Date(rentData.dropOff).getTime() -
        new Date(rentData.pickUp).getTime()) /
        (1000 * 60 * 60 * 24) <
      1
    ) {
      NotificationManager.error(
      ...
      if (rentData.driverReq) {
        await axios
          .post(
            "https://desolate-sea-14156.herokuapp.com/rent/searchDriversAvailable",
            rentData
          )
          .then(async (res) => {
            if (res.data._id) {
              await axios
                .post("https://desolate-sea-14156.herokuapp.com/rent", {
                  ...rentData,
                  driver_Id: res.data._id,
                  driver_name: res.data.name,
                })
                .then((response) =>
                  router.push(`/rent/available_cars/${response.data._id}`)
                )
                .catch((error) => {
                  setloading(false);
                  NotificationManager.error(
              ...
              NotificationManager.error(
                "No hay choferes disponibles para esa fecha",
                "Error",
                4000
              );
        ...
        await axios
          .post("https://desolate-sea-14156.herokuapp.com/rent", rentData)
          .then((response) =>
            router.push(`/rent/available_cars/${response.data._id}`)
          )
  ...
  return (
    <>
      ...
      <main>
          ...
          <div className="form">
            <div className="inp">
              ...
              <select name="location" onChange={(e) => onchangeSelect(e)}>
                ...
                <option value="Habana Aeopuerto Terminal 2">
                  Havana Aeopuerto Terminal 2
                </option>
                <option value="Habana AeropuertoTerminal 3">
                  Havana AeropuertoTerminal 3
                </option>
              </select>
            </div>
            <div className="inp">
            ...
            <div className="inp">
              ...
              <select name="pickHour" onChange={(e) => onchangeSelect(e)}>
                ...
                {rentData.pickUp == today
                  ? hoursArray.map(
                      (hour) =>
                        new Date().getHours() < hour && (
                          <option key={hour} value={hour}>{`${hour}:00 ${
                            hour < 12 ? "AM" : "PM"
                          }`}</option>
                        )
                    )
        ...
        <h4>Autos disponibles en la empresa :</h4>
        <div className="list">
          {carList.length == 0 ? (
